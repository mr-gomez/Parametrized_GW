...
def heat_kernel(G, t):
    ...
    The heat kernel is defined as ``H = exp(-t * L)``, where ``L`` is the
    graph Laplacian matrix and ``t`` is a diffusion time parameter.
    It captures the diffusion of heat (or information) over the graph.
    ...
    G : networkx.Graph
        The input graph.
    t : float
        Diffusion time parameter controlling how far heat diffuses.
...
def shortest_paths_subsets(G, v_start, v_end):
    """
    Compute shortest path distances between subsets of nodes.
...
def all_pairs_shortest_path_weighted(G, weight="weight", progress=0.1):
    ...
    Uses Dijkstra’s algorithm (via NetworkX) to compute shortest path
    distances between all pairs of nodes. Designed for sparse graphs
    where Dijkstra’s algorithm is more efficient than Floyd–Warshall.
    ...
    dm : numpy.ndarray
        ...
        containing pairwise shortest path distances.
    """
...
def cycle_of_generators_variable(num_groups, generator, arglist):
    """
    Construct a cycle of graphs generated by a provided generator function.

    Each graph is created using parameters from ``arglist`` and connected
    in a cycle by joining the 0-th vertices of consecutive graphs.

    Parameters
    ----------
    num_groups : int
        Number of graphs to generate and connect in a cycle.
    generator : callable
        A function that generates a NetworkX graph given arguments.
    arglist : list of tuple
        List of argument tuples, one for each graph generation call.

    Returns
    -------
    G : networkx.Graph
        A graph composed of ``num_groups`` subgraphs arranged in a cycle.
    """
    n_nodes = 0
    t_prev = 0
...
def cycle_of_generators(num_groups, generator, *args):
    """
    Create a cycle of identical graphs generated by a single generator.

    Parameters
    ----------
    num_groups : int
        Number of graphs to generate and connect.
    generator : callable
        Function that generates a NetworkX graph.
    *args : tuple
        Arguments passed to the generator for each graph.

    Returns
    -------
    G : networkx.Graph
        Combined graph with ``num_groups`` identical subgraphs in a cycle.
    """
...
def cycle_of_generators_fun(generator):
    ...
    ----------
    generator : callable
        A NetworkX graph generator function.

    Returns
    -------
    fun : callable
        Function taking ``num_groups`` and ``*args`` to generate a cycle
        of graphs using the fixed generator.
    """
...
def nested_cycles(*args):
    """
    Create a nested cycle of graphs.

    Each level of the nesting represents a cycle whose nodes are
    themselves cycles, recursively defined.
    ...
    *args : int
    ...
    -------
    G : networkx.Graph
        The resulting nested cycle graph.
    """
    num_args = len(args)
...
def wedge_of_generators(generator, arglist):
    """
    Create a wedge (shared vertex) of multiple generated graphs.

    Each graph is created using ``generator(*arglist[i])`` and all
    graphs are joined at their 0-th vertex.
    ...
    arglist : list of tuple
        List of argument tuples for each graph.

    Returns
    -------
    G : networkx.Graph
        Combined graph with all subgraphs joined at a single vertex.
    """
...
def wedge_of_generators_fun(generator):
    ...
    -------
    fun : callable
        Function that takes ``arglist`` and generates a wedge of graphs
...
def pos_cycle(m, R=1, c=[0, 0], t0=np.pi / 2):
    ...
    t0 : float, optional
        Starting angle offset in radians (default is π/2).

    Returns
    -------
    pos : numpy.ndarray
        Array of shape ``(m, 2)`` with x, y coordinates of each node.
    """
...
def pos_cycle_of_graphs(n, pos_list, R=4, t0=np.pi):
    """
    Arrange subgraph positions around an outer cycle.
    ...
    pos_list : list of numpy.ndarray
        List of position arrays, one per subgraph.
    R : float, optional
        Radius of the outer cycle (default is 4).
    t0 : float, optional
        Angular offset of the outer cycle in radians (default is π).

    Returns
    -------
    pos_all : numpy.ndarray
        Combined array of node positions for all subgraphs.
    """
...
def pos_cycle_uniform(n, pos_0, R=4, t0=np.pi):
    """
    Generate positions for a uniform cycle of identical subgraphs.

    Parameters
    ----------
    n : int
        Number of identical subgraphs.
    pos_0 : numpy.ndarray
        Node positions for a single subgraph.
    R : float, optional
        Radius of the outer cycle (default is 4).
    t0 : float, optional
        Angular offset in radians (default is π).

    Returns
    -------
    pos_all : numpy.ndarray
        Combined position array for all subgraphs.
    """
...
def pos_nested_cycles(*args, scale=4):
    """
    Compute node positions for a nested cycle structure.

    Each level corresponds to a cycle, recursively arranged inside
    the next. The scaling factor controls the relative distance
    between cycles.

    Parameters
    ----------
    *args : int
        Sequence of integers defining the number of nodes per cycle,
    ...
    pos_new : numpy.ndarray
        Node positions for the nested cycle graph.
    """
    num_args = len(args)
